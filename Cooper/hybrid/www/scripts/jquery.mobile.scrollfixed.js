...
function scrollfixed (options) {
	...
	that.element = $('div[data-role="content"]').get(0);
...
scrollfixed.prototype = {
	...
	touchStart: function(e) {
		...
		that.scrolling = true;		// This is probably not needed, but may be useful if scrollfixed is used in conjuction with other frameworks
...
var has3d = ('WebKitCSSMatrix' in window && 'm11' in new WebKitCSSMatrix()),
	...
	// Expose scrollfixed to the world
	window.scrollfixed = scrollfixed;

	function loaded() {
		setHeight();
		new scrollfixed({desktopCompatibility:true});
	}

	function setHeight() {
		var headerH = $('div[data-role="header"]').get(0).offsetHeight,
			footerH = $('div[data-role="footer"]').get(0).offsetHeight,
			wrapperH = window.innerHeight - headerH - footerH;
		$('div[data-role="scrollwrapper"]').get(0).style.height = wrapperH + 'px';
	}
